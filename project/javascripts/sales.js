...
app.get('', async (req, res) => {
    ...
    const resultado = await connection.query('SELECT * from film');
    res.json(resultado);
});
...
function crearTarjetasProductosCarrito() {
    ...
    const productos = JSON.parse(localStorage.getItem("http://localhost:3000/database"));
...
function revisarMensajeVacio() {
    const productos = JSON.parse(localhost.getItem("database"));
